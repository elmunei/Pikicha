...
//  pikicha
//
//  Created by Elvis Tapfumanei on 6/2/17.
...
class posts: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        let backBtn = UIBarButtonItem(image: UIImage(named: "back.png"), style: .plain, target: self, action: #selector(posts.back(_:)))
        self.navigationItem.leftBarButtonItem = backBtn
        
        // swipe to go back
        let backSwipe = UISwipeGestureRecognizer(target: self, action: #selector(posts.back(_:)))
        backSwipe.direction = UISwipeGestureRecognizerDirection.right
        self.view.addGestureRecognizer(backSwipe)
        
        NotificationCenter.default.addObserver(self, selector: #selector(posts.refresh), name: NSNotification.Name(rawValue: "liked"), object: nil)
