...
//  pikicha
//
//  Created by Elvis Tapfumanei on 6/10/17.
...
class postVC: UIViewController, UIImagePickerControllerDelegate,TGCameraDelegate,UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // hide kyeboard tap
        let hideTap = UITapGestureRecognizer(target: self, action: #selector(postVC.hideKeyboardTap))
        hideTap.numberOfTapsRequired = 1
        self.view.isUserInteractionEnabled = true
        self.view.addGestureRecognizer(hideTap)
        
        // select image tap
        let picTap = UITapGestureRecognizer(target: self, action: #selector(postVC.selectImg))
