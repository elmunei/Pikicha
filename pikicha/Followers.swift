...
class Followers: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // title at the top
        self.navigationItem.title = category.lowercased()
        
        
        // new back button
        self.navigationItem.hidesBackButton = true
        let backBtn = UIBarButtonItem(image: UIImage(named: "back.png"), style: .plain, target: self, action: #selector(Followers.back(_:)))
        self.navigationItem.leftBarButtonItem = backBtn
        
        // swipe to go back
        let backSwipe = UISwipeGestureRecognizer(target: self, action: #selector(Followers.back(_:)))
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.usernameLbl.text = usernameArray[indexPath.row].lowercased()
        ...
        query.countObjectsInBackground (block: { (count, error) -> Void in
            if error == nil {
                if count == 0 {
                    ...
                    cell.followBtn.backgroundColor = UIColor(red: 8/255, green: 165/255, blue: 137/255, alpha: 1.0)
                }
            }
        })
